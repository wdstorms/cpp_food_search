...
std::vector<std::tuple<int, std::pair<boost::dynamic_bitset<>, boost::dynamic_bitset<>>, std::vector<std::string>>> PacmanGraph::get_successors(std::pair<boost::dynamic_bitset<>, boost::dynamic_bitset<>> state, std::vector<std::string> actions, int cost) {
    std::vector<std::tuple<int, std::pair<boost::dynamic_bitset<>, boost::dynamic_bitset<>>, std::vector<std::string>>> successors;
    for (auto n : nodes[state.first]) {
        ...
        a.push_back(dirs[d]);
        successors.push_back({cost - 1, {n, state.second & (~n)}, a});
    }
    return successors;
}